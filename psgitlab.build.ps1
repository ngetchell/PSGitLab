...
Task Pester -inputs { Get-ChildItem -Path "$projectRoot\$ModuleName\","$projectRoot\Tests\" -Recurse | Where-Object { -not $_.PSisContainer } } -outputs $PesterResultsFile Build, {
    ...
    $testResults = Invoke-Pester -Path $tests -PassThru -OutputFile $PesterResultsFile -ExcludeTag 'Integration'
...
Task Integration  BeforeIntegration, {
    ...
    $testResults = Invoke-Pester -Path $tests -PassThru -OutputFile $PesterResultsFile -Tag 'Integration'

    if ($testResults.FailedCount -gt 0) {
        $testResults | Format-List
        throw 'One or more Integration tests failed. Build cannot continue!'
    }    
}
